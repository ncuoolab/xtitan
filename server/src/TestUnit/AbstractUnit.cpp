...
/**
 * @note maybe customize point
 * clients maybe need to decide how to spawn their program. if they do, they
 * need to kill ones, too.
 */
void AbstractUnit::Private::spawn() {
	...
	for( int i = 0; i < this->testCase.getClients(); ++i ) {
		...
		p->start( software.filePath(), args );
		this->children_.append( p );
	}
}
