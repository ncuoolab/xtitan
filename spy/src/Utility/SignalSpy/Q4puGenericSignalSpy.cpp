...
#include "Q4puGenericSignalSpy.hpp"
...
bool q4pugss_GetMethodString(QObject* caller, int method_index, void **argv,  QString &string)
{
	...
	//			.arg(mo->className());
	//string = QString("%1:").arg(methodType[(int)m.methodType()]);
	string = QString("%1.%2(")
				.arg(caller->objectName().isNull()?"noname":caller->objectName())
	...
	for(int i=0; i<pNames.count(); i++) {
		//string += QString("%1=%2").arg(q4pugss_value(pTypes.at(i), argv[i+1]));
		if( pTypes.at(i) == "QString"){
			string += "'"+q4pugss_value(pTypes.at(i), argv[i+1]) + "'";
		}else{
			string += q4pugss_value(pTypes.at(i), argv[i+1]);
		}
...
const QString q4pugss_value(const char *type, void *argv)
{
	...
	if( v.type() ){
		...
		return QString("%2").arg(v.toString());
	}
